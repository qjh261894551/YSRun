...
@interface MCPaceCalculateFunc ()

@property (nonatomic, strong) MCTimeLocationArray *timeLocationArray;
...
@implementation MCPaceCalculateFunc
...
- (void)calculate
{
    ...
    {
        MCPaceSectionDataModel *sectionData = [self getPaceSectionDataWithArray:array];
    ...
    {
        MCPaceSectionDataModel *sectionData = self.sectionDataArray[i];
...
- (void)updatePaceValue
{
    ...
    MCPaceSectionDataModel *sectionData = [self.sectionDataArray firstObject];
...
- (MCPaceSectionDataModel *)getPaceSectionDataWithArray:(NSArray<MCTimeLocationModel *> *)array
{
    // 根据数组数据做计算
    MCPaceSectionDataModel *dataModel = [MCPaceSectionDataModel new];
    ...
    {
        MCTimeLocationModel *first = [array firstObject];
        MCTimeLocationModel *last = [array lastObject];
...
- (NSArray *)splitTimeLocationModelArray:(NSArray *)modelArray byDistance:(CGFloat)distance
{
    ...
    MCTimeLocationModel *lastModel = [modelArray firstObject];
    ...
    for (NSInteger i = 1; i < count; i ++)
    {
        MCTimeLocationModel *currentModel = modelArray[i];
        CGFloat d = [MCMapCalculateFunc distanceBetweenCoordinate1:lastModel.coordinate coordinate2:currentModel.coordinate];
...
- (CGFloat)calculateDistance:(NSArray<MCTimeLocationModel *> *)modelArray
{
    // 计算所包含的位置数据点的总距离
    NSMutableArray *annotationArray = [NSMutableArray array];
    for (MCTimeLocationModel *model in modelArray)
    {
        MCMapAnnotation *annotation = [[MCMapAnnotation alloc] initWithCoordinate:model.coordinate];
