...
#import "MCPaceDataItem.h"
#import "MCAppMacro.h"
#import "MCDevice.h"

@interface MCPaceDataTableView ()
...
@implementation MCPaceDataTableView
...
{
    // 给标签加上高度约束
    NSArray *constraints = [self getConstraintsWithView:label height:MCPaceDataTableViewLabelHeight];
...
- (void)addItems
{
    MCPaceDataItem *previousItem = nil;
    
    for (MCPaceSectionDataModel *model in self.sectionDataModelArray)
    {
        MCPaceDataItem *item = [[[UINib nibWithNibName:@"MCPaceDataItem" bundle:nil] instantiateWithOwner:self options:nil] objectAtIndex:0];
        ...
        if (previousItem)
        {
            NSArray *constraints_V = [NSLayoutConstraint constraintsWithVisualFormat:@"V:[previousItem][item(height)]"
                                                                             ...
                                                                             metrics:@{@"height" : @(MCPaceDataTableViewItemHeight)}
