...
@interface MCThirdPartShareSelectView ()

@property (nonatomic, strong) NSMutableArray *shareButtons;

@end

@implementation MCThirdPartShareSelectView
...
{
    // 微信朋友圈
    [self.delegate shareType:MCShareSelectTypeWXCircleOfFriends];
}

- (void)shareToWeixinFriends
{
    // 微信好友
    [self.delegate shareType:MCShareSelectTypeWXFriends];
}

- (void)shareToWeibo
{
    // 新浪微博
    [self.delegate shareType:MCShareSelectTypeSinaWeibo];
}

- (void)shareToQQFrineds
{
    // QQ好友
    [self.delegate shareType:MCShareSelectTypeQQFriends];
}

- (void)shareToQZone
{
    // QQ空间
    [self.delegate shareType:MCShareSelectTypeQQZone];
}
