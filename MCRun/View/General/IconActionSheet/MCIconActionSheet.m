...
@implementation MCIconActionSheet
...
- (void)sheetAddItems
{
    ...
    for (NSInteger i = 0; i < [self.dictArray count]; i++)
    {
        ...
        UIImage *icon = [dict valueForKey:MCIconActionSheetItemIconKey];
        NSString *text = [dict valueForKey:MCIconActionSheetItemTextKey];
        id object = [dict valueForKey:MCIconActionSheetItemObjectKey];
        
        NSString *selectorString = [dict valueForKey:MCIconActionSheetItemSelectorStringKey];
