...
typedef NS_ENUM(NSInteger, MCGraphInterceptionType)
{
    MCGraphInterceptionTypeTop = 1,
    MCGraphInterceptionTypeMiddle,
    MCGraphInterceptionTypeBottom
};

@interface MCGraphCanvas ()

@property (nonatomic, strong) MCGraphData *graphData;
...
- (UIImage *)getGraphImageWithSize:(CGSize)size
{
    ...
    UIImage *topImage = [self getInterceptionPartWithType:MCGraphInterceptionTypeTop backgroundColor:AnaerobicExerciseColor lineColor:AnaerobicExerciseLineColor];
    UIImage *middleImage = [self getInterceptionPartWithType:MCGraphInterceptionTypeMiddle backgroundColor:EfficientReduceFatColor lineColor:EfficientReduceFatLineColor];
    UIImage *bottomImage = [self getInterceptionPartWithType:MCGraphInterceptionTypeBottom backgroundColor:JoggingColor lineColor:JoggingLineColor];
...
- (UIImage *)getInterceptionPartWithType:(MCGraphInterceptionType)type backgroundColor:(UIColor *)backgroundColor lineColor:(UIColor *)lineColor
{
...
- (CGRect)getInterceptionRectWithType:(MCGraphInterceptionType)type
{
    // 获取需要截取的rect
    ...
    CGFloat originY = 0;
    if (type == MCGraphInterceptionTypeMiddle)
    {
        originY = height / 3;
    }
    else if (type == MCGraphInterceptionTypeBottom)
    {
        originY = height / 3 * 2;
    }
...
- (NSArray *)getPoints
{
    ...
    {
        MCGraphPoint *graphPoint = [self.graphData graphPointAtIndex:i];
...
- (CGPoint)pointFromGraphPoint:(MCGraphPoint *)graphPoint
{
