...
#import "MCCalendar.h"
#import "MCCalendarDayItem.h"
#import "NSDate+MCDateLogic.h"
#import "MCDayItemModel.h"
#import "MCDatabaseManager.h"
#import "MCUtilsMacro.h"
#import "MCAppMacro.h"

@interface MCCalendar () <UIScrollViewDelegate, MCCalendarDayItemDelegate>
...
@property (nonatomic, strong) MCDatabaseManager *databaseManager;
...
- (void)setup
{
    ...
    self.databaseManager = [MCDatabaseManager new];
...
- (void)resetDayItemsFrame:(NSArray *)dayItems
{
    ...
    for (NSInteger i = 0; i < [dayItems count]; i++)
    {
        ...
        MCCalendarDayItem *dayItem = dayItems[i];
        dayItem.frame = frame;
    }
}
...
- (void)daysView:(UIView *)daysView addDayItems:(NSArray *)dayItems
{
    ...
    {
        MCLog(@"daysView为空，或dayItems数组个数不正确");
        return;
    }
...
- (NSArray *)getDayItemArray
{
    ...
    for (NSInteger i = 0; i < dayItemCount; i++)
    {
        MCCalendarDayItem *dayItem = [MCCalendarDayItem new];
...
- (void)setupDayItemArray:(NSArray *)dayItemArray withDataArray:(NSArray *)dataArray
{
    ...
    {
        MCLog(@"dayItemArray、dataArray数组个数不正确");
        return;
    }
    ...
    for (NSInteger i = 0; i < dayItemCount; i++)
    {
        MCCalendarDayItem *dayItem = dayItemArray[i];
        MCDayItemModel *model = dataArray[i];
...
- (NSArray *)getDataArrayWithDate:(NSDate *)date
{
    ...
    for (NSInteger i = 0; i < numberOfDaysInLastMonth; i++)
    {
        ...
        MCDayItemModel *model = [self getDayItemModelWithDate:date isCurrentMonth:NO];
    ...
    for (NSInteger i = 0; i < numberOfDaysInMonth; i++)
    {
        ...
        MCDayItemModel *model = [self getDayItemModelWithDate:date isCurrentMonth:YES];
        [dataArray addObject:model];
    }
    ...
    if ([self compareDate1:date date2:self.selectedDate])
    {
        ...
        MCDayItemModel *model = dataArray[selectedIndex];
        model.selected = YES;
    }
...
- (MCDayItemModel *)getDayItemModelWithDate:(NSDate *)date isCurrentMonth:(BOOL)isCurrentMonth
{
    MCDayItemModel *model = [MCDayItemModel new];
...
//    MCLog(@"targetContentOffset x = %@, y = %@, velocity = %@", @((*targetContentOffset).x), @((*targetContentOffset).y), @(velocity.x));
...
#pragma mark - MCCalendarDayItemDelegate
