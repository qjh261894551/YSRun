...
@interface MCNetworkRequest ()

@end

@implementation MCNetworkRequest

static MCNetworkRequest *_instance;
...
+ (instancetype)shareNetworkRequest
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[MCNetworkRequest alloc] init];
    });
    return _instance;
}
...
- (void)requestFailureWithError:(NSError *)error delegate:(id<MCNetworkRequestDelegate>)delegate
{
    // 网络请求失败
...
- (void)acquireCaptchaWithPhoneNumber:(NSString *)phoneNumber
                             delegate:(id<MCNetworkRequestDelegate>)delegate
{
    // 获取验证码
    ...
    if (responseObject && [responseObject isKindOfClass:[NSDictionary class]])
    {
        ...
        switch (result)
        {
            case 1:
            {
                ...
                MCUserInfoResponseModel *userInfoResponseModel = [self userInfoResponseModelFromDataDictionary:dict];
    ...
    if (responseObject && [responseObject isKindOfClass:[NSDictionary class]])
    {
        ...
        switch (result)
        {
            case 1:
            {
                ...
                MCUserDatabaseModel *userDatabaseModel = [self userDatabaseModelFromDictionary:dict uid:uid lastTime:lastTime];
        ...
        switch (result)
        {
            case 1:
            {
                ...
                for (NSInteger i = 0; i < [datas count]; i++)
                {
                    ...
                    MCRunDatabaseModel *model = [self runDatabaseModelFromDataDictionary:dict uid:uid];
...
- (MCUserInfoResponseModel *)userInfoResponseModelFromDataDictionary:(NSDictionary *)dataDict
{
    MCUserInfoResponseModel *userInfoResponseModel = [MCUserInfoResponseModel new];
...
- (MCRunDatabaseModel *)runDatabaseModelFromDataDictionary:(NSDictionary *)dataDict uid:(NSString *)uid
{
...
- (MCUserDatabaseModel *)userDatabaseModelFromDictionary:(NSDictionary *)dict uid:(NSString *)uid lastTime:(NSString *)lastTime
{
...
- (NSDictionary *)requestDictFromThirdPartLoginModel:(MCThirdPartLoginResponseModel *)model
{
    // 第三方登录向服务器请求的参数
