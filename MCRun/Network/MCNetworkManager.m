...
#import "MCUploadRunDataRequestModel.h"
#import "MCRunDatabaseModel.h"
#import "MCModifyPasswordRequestModel.h"
...
@interface MCNetworkManager () <MCNetworkRequestDelegate>

@property (nonatomic, strong) MCNetworkRequest *networkRequest;
...
- (id)init
{
    ...
    if (self)
    {
        self.networkRequest = [MCNetworkRequest shareNetworkRequest];
...
- (void)uploadRunData:(MCRunDatabaseModel *)runDatabaseModel
{
    MCUploadRunDataRequestModel *requestModel = [self uploadRunDataRequestFromRunDatabaseModel:runDatabaseModel];
...
- (void)userRegister:(MCRegisterInfoRequestModel *)registerInfo
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(){
...
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(){
        MCModifyPasswordRequestModel *modiyPasswordModel = [MCModifyPasswordRequestModel new];
...
- (void)setUserWithRequestModel:(MCSetUserRequestModel *)setUserRequestModel
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(){
...
- (void)thirdPartLoginWithThirdPartLoginResponseModel:(MCThirdPartLoginResponseModel *)model
{
    // 第三方登录，将对应字段向服务器进行请求
...
#pragma mark - MCNetworkRequestDelegate
...
// 用户登录
- (void)userLoginSuccessWithUserInfoResponseModel:(MCUserInfoResponseModel *)userModel
{
...
- (void)networkRequestFailureWithError:(NSError *)error
{
    ...
    [[MCTipLabelHUD shareTipLabelHUD] showTipWithText:@"网络请求超时，请检查网络连接状况"];
}

#pragma mark - private

- (MCUploadRunDataRequestModel *)uploadRunDataRequestFromRunDatabaseModel:(MCRunDatabaseModel *)runDatabaseModel
{
    MCUploadRunDataRequestModel *requestModel = [MCUploadRunDataRequestModel new];
