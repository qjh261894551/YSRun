...
#define MCBluetoothConnectScanTime      5      // 扫描一定时间后若无设备则中断扫描

@interface MCBluetoothConnect () <CBCentralManagerDelegate, CBPeripheralDelegate>
...
@property (nonatomic, copy) MCBLEConnectPeripheralStateBlock connectStateCallback;
@property (nonatomic, copy) MCExameBluetoothStateBlock examBLECallback;

@property (nonatomic, strong) NSTimer *simulationTimer;

@end

@implementation MCBluetoothConnect
...
static MCBluetoothConnect *_instance;
...
+ (instancetype)shareBluetoothConnect
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[MCBluetoothConnect alloc] init];
    });
    return _instance;
}
...
{
    [self addObserver:observer forKeyPath:MCBluetoothConnectHeartRateKey options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld context:nil];
}

- (void)removeHeartRateObserver:(id)observer
{
    [self removeObserver:observer forKeyPath:MCBluetoothConnectHeartRateKey];
}
...
- (void)scanTimerStart
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^(){
        self.timer = [NSTimer scheduledTimerWithTimeInterval:MCBluetoothConnectScanTime target:self selector:@selector(interruptScan) userInfo:nil repeats:NO];
    });
}
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central
{
    ...
    switch (central.state) {
        case CBCentralManagerStateUnknown:
            MCLog(@">>>CBCentralManagerStateUnknown");
            break;
        case CBCentralManagerStateResetting:
            MCLog(@">>>CBCentralManagerStateResetting");
            break;
        case CBCentralManagerStateUnsupported:
            MCLog(@">>>CBCentralManagerStateUnsupported");
            break;
        case CBCentralManagerStateUnauthorized:
            MCLog(@">>>CBCentralManagerStateUnauthorized");
            break;
        case CBCentralManagerStatePoweredOff:
            MCLog(@">>>CBCentralManagerStatePoweredOff 设备蓝牙开关未打开");
        ...
        case CBCentralManagerStatePoweredOn:
            MCLog(@">>>CBCentralManagerStatePoweredOn");
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
    ...
    MCLog(@">>>连接设备（%@）失败,原因:%@",[peripheral name],[error localizedDescription]);
...
{
    //    MCLog(@"Discovered service %@", service);
    for (CBService *service in peripheral.services)
    {
        MCLog(@"Discovering characteristics for service %@", service);
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    ...
    {
        MCLog(@"Discovered characteristic %@", characteristic);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    if (error)
    {
        MCLog(@"Error changing notification state: %@", [error localizedDescription]);
        return;
    }
