...
#import "MCCoordinateProcesser.h"

@interface MCMapManager () <MAMapViewDelegate>
...
@property (nonatomic, strong) MCCoordinateProcesser *processer;
...
- (id)init
{
    ...
    if (self)
    {
        ...
        self.processer = [MCCoordinateProcesser new];
...
{
    // 先移除之前绘制的路径
    [MCMapCalculateFunc addAnnotationArray:self.annotationRecordArray toMapView:self.mapView];
    [self removePreviousRoute];
    
    // 绘制最新的路径
    MCMapAnnotation *annotation = [self.annotationRecordArray lastObject];
...
{
    return [MCMapCalculateFunc totalDistance:self.annotationRecordArray];
}
...
- (void)updateSpeedWithNewCoordinate:(CLLocationCoordinate2D)newCoordinate
{
    ...
    CGFloat distance = [MCMapCalculateFunc distanceBetweenCoordinate1:self.lastLocationCoordinate coordinate2:self.currentLocationCoordinate];
