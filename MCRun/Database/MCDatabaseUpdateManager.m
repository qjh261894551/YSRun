...
#import "MCDatabaseUpdateManager.h"
...
@interface MCDatabaseUpdateManager ()
...
@implementation MCDatabaseUpdateManager

+ (void)setDefaultVersion
{
    // 第一次创建数据库时，设为初始的版本值1
    [[NSUserDefaults standardUserDefaults] setValue:[NSNumber numberWithInteger:1] forKey:MCDatabaseVersionKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

- (void)setDatabaseVersion:(NSInteger)version
{
    [[NSUserDefaults standardUserDefaults] setValue:[NSNumber numberWithInteger:version] forKey:MCDatabaseVersionKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

- (NSInteger)getDatabaseVersion
{
    NSNumber *value = [[NSUserDefaults standardUserDefaults] valueForKey:MCDatabaseVersionKey];
    if (!value)
    {
        MCLog(@"数据库版本号为空");
        return -1;
...
- (BOOL)hasSetVersion
{
    ...
    NSValue *value = [[NSUserDefaults standardUserDefaults] valueForKey:MCDatabaseVersionKey];
    if (value)
    {
        return YES;
    }
