...
#import "MCDatabaseQueue.h"
#import "MCDatabaseTableKey.h"
#import "FMDB.h"
#import "MCDatabaseTableCreator.h"
#import "MCUserDatabaseModel.h"
#import "MCRunDatabaseModel.h"
#import "MCUtilsMacro.h"
#import "MCModelReformer.h"

@interface MCDatabaseQueue ()

@property (nonatomic, strong) FMDatabaseQueue *queue;

@end

@implementation MCDatabaseQueue

static MCDatabaseQueue *_instance;
...
+ (instancetype)shareDatabaseQueue
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[MCDatabaseQueue alloc] init];
    });
    return _instance;
}
...
- (void)connect
{
    ...
    MCDatabaseTableCreator *dbCreator = [[MCDatabaseTableCreator alloc] initWithQueue:self.queue];
...
- (MCUserDatabaseModel *)getUser
{
    __block MCUserDatabaseModel *model = nil;
...
- (NSArray *)getRunInfoInDate:(NSDate *)date
{
    ...
    [self.queue inTransaction:^(FMDatabase *db, BOOL * rollback) {
        NSString *dateStr = [MCModelReformer stringFromDate:date];
        ...
        while ([queryRes next])
        {
            MCRunDatabaseModel *runData = [MCRunDatabaseModel new];
...
- (void)updateUser:(NSString *)uid withUserInfo:(MCUserDatabaseModel *)userInfo
{
    // 修改uid对应的用户信息
}
...
- (NSInteger)insertRunData:(MCRunDatabaseModel *)runData
{
    // 跑步数据存储到本地数据库
...
- (void)insertUser:(MCUserDatabaseModel *)userInfo
{
    // 用户数据存到本地
    [self.queue inTransaction:^(FMDatabase *db, BOOL * rollback) {
        NSMutableArray *arguments = [NSMutableArray array];
...
- (void)deleteHeartRateDataAndLocationDataWithUid:(NSString *)uid inDatabase:(FMDatabase *)db
{
    ...
    {
        MCRunDatabaseModel *runData = runDataArray[i];
