...
@property (nonatomic, strong) MCDatabaseQueue *dbQueue;
...
- (id)init
{
    ...
    if (self)
    {
        self.dbQueue = [MCDatabaseQueue shareDatabaseQueue];
    }
...
{
    MCUserInfoModel *userInfo = [MCUserInfoModel new];
    
    MCUserDatabaseModel *databaseModel = [self.dbQueue getUser];
    if (databaseModel)
    {
...
- (void)setupUserGrage:(MCUserInfoModel *)user
{
    // 计算等级相关数据
...
- (void)addUser:(MCUserDatabaseModel *)userDatabaseModel
{
...
- (NSInteger)addRunData:(MCRunDatabaseModel *)model
{
    // 将跑步数据存储到数据库
...
{
    MCUserDatabaseModel *model = [self.dbQueue getUser];
...
- (NSInteger)getRunStarRatingWithDate:(NSDate *)date
{
    ...
    {
        for (MCRunDatabaseModel *model in runDataArray)
        {
